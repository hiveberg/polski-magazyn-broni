...
const createSchema = z.object({ firstName: z.string().trim().min(2).max(80), lastName: z.string().trim().min(2).max(100), login: z.string().trim().min(3).max(80).regex(/^[a-zA-Z0-9._-]+$/), temporaryPassword: passwordSchema, role: z.enum(["ADMIN", "AUTHORIZED"]), isAuthorized: z.boolean() });
