...
export function UserStatusButton({ id, active }: { id: string; active: boolean }) {
  const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const router = useRouter(); return <div className="table-action">{error && <span className="field-error">{error}</span>}<button className={`button ${active ? "danger" : ""}`} disabled={loading} onClick={async () => { setLoading(true); setError(""); const response = await fetch("/api/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, active: !active }) }); const body = await response.json(); if (!response.ok) setError(body.error); else router.refresh(); setLoading(false); }}>{active ? "Dezaktywuj" : "Aktywuj"}</button></div>;
}
