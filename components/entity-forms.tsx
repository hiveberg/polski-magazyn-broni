...
type AmmoIssuePayload = { bookId: string; sourceBookId?: string; caliberId: string; ammunitionType: string; quantity: number; recipientName: string; recipientReference?: string; parentIssueId?: string };
...
export function AddWeaponForm({ books, calibers, documents }: { books: BookOption[]; calibers: CaliberOption[]; documents: DocumentOption[] }) {
  const router = useRouter(); const [version, setVersion] = useState(0); const [created, setCreated] = useState<{ id: string; registryRef: string; name: string; book: string } | null>(null);
  const operation = usePinMutation<Record<string, unknown>>(async (payload, pin) => { const response = await requestJson("/api/weapons", { ...payload, pin }); setCreated({ id: response.weapon.id, registryRef: response.weapon.registryRef, name: String(payload.name), book: books.find((book) => book.id === payload.bookId)?.series ?? "—" }); setVersion((value) => value + 1); router.refresh(); });
  ...
  return <><form key={version} className="form-stack" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); operation.request({ bookId: data.get("bookId"), caliberId: data.get("caliberId"), documentId: data.get("documentId"), name: data.get("name"), brand: data.get("brand"), productionYear: data.get("productionYear") || undefined, weaponSeries: data.get("weaponSeries") || undefined, serialNumber: data.get("serialNumber"), otherIdentifyingMarks: data.get("otherIdentifyingMarks") || undefined, accessories: data.get("accessories") || undefined, type: data.get("type"), acquisitionBasis: data.get("acquisitionBasis"), acquisitionDate: data.get("acquisitionDate") || undefined, registeredAt: data.get("registeredAt"), certificateNumber: data.get("certificateNumber") || undefined, notes: data.get("notes") || undefined }); }}><Messages error={operation.error} /><div className="form-grid"><BookPicker label="Księga broni" books={books} /><div className="field"><label htmlFor="weapon-type">Rodzaj</label><select className="select" id="weapon-type" name="type"><option value="HANDGUN">Broń krótka</option><option value="LONG_GUN">Broń długa</option></select></div><TextField label="Nazwa ewidencyjna" name="name" required /><TextField label="Marka" name="brand" required /><CaliberPicker calibers={calibers} /><TextField label="Rok produkcji" name="productionYear" type="number" /><TextField label="Seria broni" name="weaponSeries" /><TextField label="Numer broni" name="serialNumber" required /><TextField label="Numer świadectwa broni" name="certificateNumber" /><TextField label="Wyposażenie dodatkowe" name="accessories" /><TextField label="Podstawa nabycia" name="acquisitionBasis" required full /><div className="field full"><DocumentPicker documents={documents} label="Dokument nabycia" /></div><TextField label="Data nabycia" name="acquisitionDate" type="datetime-local" /><TextField label="Data zewidencjonowania" name="registeredAt" type="datetime-local" defaultValue={nowInput()} required /><TextArea label="Inne cechy identyfikacyjne" name="otherIdentifyingMarks" /><TextArea label="Uwagi" name="notes" /></div><Submit>Zapisz i potwierdź PIN-em</Submit></form>{operation.dialog}</>;
}
  ...
  const newIssueContent = created
    ...
    : !calibers.length
      ...
      : <form className="form-stack" onSubmit={(event) => {
          ...
          operation.request({ bookId: String(data.get("bookId") ?? ""), sourceBookId: sourceBookId || undefined, caliberId, ammunitionType: String(data.get("ammunitionType") ?? ""), quantity: quantityNumber, recipientName: String(data.get("recipientName") ?? ""), recipientReference: String(data.get("recipientReference") ?? "") || undefined });
        }}>
          ...
          <div className="form-grid">
            ...
            <TextField label="Odbiorca" name="recipientName" required />
            <TextField label="Dokument / identyfikator odbiorcy" name="recipientReference" />
